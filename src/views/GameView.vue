      ...
      <div class="m-10">
        ...
        <ul class="py-5 my-5 border-y border-gray-600">
          <li class="flex">
            ...
            <p class="italic font-semibold">
              ...
              <input v-if="editing" class="-ml-2 rounded-lg bg-darkBackground text-white px-2 w-full" v-model="editingForm.nicknameIngame" placeholder="Nickname in-game" >
            </p>
          </li>
          <li class="flex">
            ...
            <p class="italic font-semibold">
              ...
              <input v-if="editing" class="-ml-2 rounded-lg bg-darkBackground text-white px-2 w-full" v-model="editingForm.hoursPlayed" type="number" placeholder="Hours played" >
            </p>
          </li>
          <li class="flex" v-if="game.rank || editing">
            ...
            <p class="italic font-semibold">
              ...
              <input v-if="editing" class="-ml-2 rounded-lg bg-darkBackground text-white px-2 w-full" v-model="editingForm.rank" placeholder="Rank" >
            </p>
          </li>
          <li class="flex" v-if="game.server|| editing">
            ...
            <p class="italic font-semibold">
              ...
              <input v-if="editing" class="-ml-2 rounded-lg bg-darkBackground text-white px-2 w-full" v-model="editingForm.server" placeholder="Server" >
            </p>
          </li>
        </ul>
        <div v-if="game.comments || editing">
          ...
          <p class="mt-1 ml-5">
            ...
            <textarea v-if="editing" class="-ml-2 rounded-lg bg-darkBackground text-white px-2 w-full min-h-[150px]" v-model="editingForm.comments" placeholder="Comment" />
          </p>
        </div>

        <div class="ml-auto w-fit my-5" v-if="editing">
          <button v-if="!newGame" @click="editing = !editing" class="px-5 border-gray-600 hover:text-white">Cancel</button>
          <button-sub-component v-if="!newGame" @buttonClick="updateGame" name="Submit"/>
          <button-sub-component v-if="newGame" @buttonClick="updateGame" name="Add new"/>
        </div>
        
      </div>
      <div v-if="isTheProfileOwner" class="absolute bottom-5 w-1/2 flex justify-center text-xl text-gray-400">
        <div v-if="!editing">
          <button @click="enableEditing" class="px-5 border-r border-gray-600 hover:text-white">Edit</button>
          <button @click="deleteGame" class="px-5 hover:text-white">Delete</button>
        </div>
      </div>
    </div>
    </div>
